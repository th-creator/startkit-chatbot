<template>
    <div class="">
      ...
      <div class="bot-wrapper" @click="showChat = true">
        <img src="/images/OIP.jpg" alt="" class="bot">
      </div>
      <div v-if="showChat == true" class="convo">
        <div class="convo-title grid-center">
          <!-- <ChatSVG /> -->
          <span @click="showChat = false">X</span>
        </div>
        <div class="convo-body" id="convoBody">
          <div class="convo-wrapper left-convo align">
            <img src="/images/OIP.jpg" alt="" class="convo-img">
            <p>Welcome to AI</p>
          </div>
  ...
  const ai_message = ref("hi there");
  const user_message = ref("");
  const last_message = ref("");
  const waiting_message = ref("");
  const word_balance = ref(null);
  const messages = ref([]);
  const textInput = ref("");
  ...
  const setConversation = (e) => {
        e.preventDefault()
        last_message = user_message.value
        const message = user_message.value
        user_message.value = ""
        waiting_message.value = "..."
